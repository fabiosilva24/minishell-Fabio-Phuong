...
char    *get_exit_variable_value(char **envp, char *line, int len, int last_exit_status)
{
	...
	while (envp[++i])
	{
		if (!ft_strncmp(var_name, envp[i], max(ft_len_eq(envp[i]),
				ft_strlen(var_name))))
		{
			free(var_name);
...
void    replace_exit_status(char **line, int start, int len, char *value, int *i)
{
	...
	new_line = malloc(sizeof(char) * (ft_strlen(*line) + value_len));
	if (!new_line)
		return ;
...
void    extract_and_replace_exit_status(int *i, char **line, char **envp, int last_exit_status)
{
	...
	value = get_exit_variable_value(envp, *line + (*i - count), count + 1, last_exit_status);
...
void    expand_exit_status(char **line, char **envp, int last_exit_status)
{
    ...
    while ((*line)[i] != '\0')
    {
        ...
        if ((*line)[i] == '\'')
            single_quote_count++;
        if ((single_quote_count % 2 == 0 || double_quote_count % 2 == 1) && (*line)[i] == '$')
