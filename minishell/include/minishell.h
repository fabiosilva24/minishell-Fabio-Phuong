...
void					exit_code(t_minishell *minishell, char **command,
							char **argv);
...
void					handle_sigint_with_shell(int sig, siginfo_t *info,
							void *context);
...
char					**update_pwd_oldpwd(char **envp, char *old_path,
							int *status);
char					**change_directory(char **args, int if_cdcmd,
...
char					**change_envp(char *new, char **args, int i,
							char **envp);
...
char					*find_executable(char **paths, char **cmd_flags,
							int *status);
...
void					expand_exit_status(char **line, char **envp,
...
char					**convert_tokens_to_argv_until_pipe(t_token *tokens,
							int token_count);
...
char					*find_env_value(const char *var_name,
							char **environment);
...
int						copy_env_var(char **new_mass, char **envp, int i,
							int j);
...
char					**update_env_vars(char **envp, char *pwd_var,
							char *oldpwd_var);
void					setup_pipe_io(int pipe_in, int pipe_out);
void					handle_redirectionss(char **arg, int token_count);
void					execute_child_process1(t_token *cmd_tokens,
							t_minishell *shell, int pipe_in, int pipe_out);
void					execute_piped_command(t_token *cmd_tokens,
							t_minishell *shell, int pipe_in, int pipe_out);
void					free_line_andtoken(t_token *tokens, char *line,
							t_minishell *shell);
void					line_history(char *line);
void					ft_exit(t_minishell *shell);
void					handle_quotes(char *line, int *in_double_quotes,
