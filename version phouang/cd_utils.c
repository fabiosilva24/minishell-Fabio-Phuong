...
void handle_relative_path(char **args, char **envp)
{
    ...
    if (!args[1] || !ft_strcmp(args[1], "~") || !ft_strncmp(args[1], "~/", 2))
        path = resolve_home_path(args[1], envp);
    else if (!ft_strcmp(args[1], "-"))
    {
        path = get_cd_env_value(envp, "OLDPWD");
        if (!path)
            return errmsg("minishell: cd: ", NULL, "OLDPWD not set", -1);
    }
    else
        return handle_absolute_path(args[1]);
    if (!path)
        return;
    if (ft_strlen(path) >= MAX_PATH_LEN)
